...
class MiningVietnamArticleScraper extends PageArticleScraper(MiningVietnamDomain) {
  ...
  def scrape(browser: Browser, page: Page, html: String): ArticleScrape = {
    ...
    val dateline = (graph(2) \ "datePublished").extract[String]
    val bylineOpt = graph.arr
      .find { jValue =>
        (jValue \ "@type").extract[String] == "Person"
      }
      .map { jValue => (jValue \ "name").extract[String] }
    val paragraphs = doc >> elementList("div.des_pageDefault > p")
    val text = paragraphs
      .map { paragraph =>
        paragraph.text.trim
      }
