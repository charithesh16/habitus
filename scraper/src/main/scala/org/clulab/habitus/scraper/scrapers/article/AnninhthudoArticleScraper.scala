...
class AnninhthudoArticleScraper extends PageArticleScraper (AnninhthudoDomain){
  ...
  def scrape(browser: Browser, page: Page, html: String): ArticleScrape = {
    ...
    val dateline = (doc >> elementList("div.article__meta  span.time > time")).headOption.map(_.attr("datetime"))
    val bylineOpt = (doc >> elementList("div.article__meta span.author")).headOption.map(_.text)
