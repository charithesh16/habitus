...
class BNewsArticleScraper extends PageArticleScraper (BNewsDomain){
  ...
  def scrape(browser: Browser, page: Page, html: String): ArticleScrape = {
    ...
    val dateline = (doc >> elementList("span.post-time")).headOption.map(_.text)
    val bylineOpt = (doc >> elementList("span.post-author")).headOption.map(_.text)

    val paragraphs = doc >> elementList("div.post-ct-entry > p")
    var text = ""
    if (paragraphs.isEmpty) {
      text = (doc >> element("div.post-ct-entry")).text
    } else {
      text = paragraphs
        .map { paragraph =>
          paragraph.text.trim
