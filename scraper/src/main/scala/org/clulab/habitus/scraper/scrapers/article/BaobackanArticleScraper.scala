...
class BaobackanArticleScraper extends PageArticleScraper (BaobackanDomain){
  ...
  def scrape(browser: Browser, page: Page, html: String): ArticleScrape = {
    ...
    val dateline = (doc >> elementList("div.article__meta div.time > time")).headOption.map(_.attr("datetime"))
    val bylineOpt = (doc >> elementList("div.article__author div.author div.info > a")).headOption.map(_.text)
